...
public class PortalImpl
{
    ...
    public ClientResponse payment (PaymentRequest request, int environment)
    {
        ...
        if (request.getClientData().getGuid() != null)
        {
            ...
            transaction.getRequest().setCustomer (custProfile);
...
////        lresponse.setClient (client);
    ...
    private ClientResponse noncePayment (TransactionRecord ltransaction, PaymentRequest request)
    {
        ClientResponse lresponse = new ClientResponse();
        Collection<IAccount> accounts = new ArrayList<IAccount>();
        Collection<IInvoice> invoices = new ArrayList<IInvoice>();
        ...
        ltransaction.setRefId (request.getClientData().getGuid());
        ltransaction.setMode (MODE_PROFILE);
        ltransaction.setCommand (CMD_VERIFY);

        ltransaction = methods.requestTransaction (ltransaction, params);
        client = (Client)ltransaction.getRequest().getClient();

        if (ltransaction.getResponse().getErrorCode() == TRANSACTION_SUCCESS)
        {
            ltransaction.setMode (METH_PAYMENT);
            ltransaction.setCommand (CMD_NONCEPAY);
            ltransaction.getRequest().setTransactionType (IPaymentSvc.TTYPE_AUTH_CAPTURE);

            try
            {
                ltransaction.getRequest().setAmount (Double.valueOf (request.getAccount().getAmount()));
                ltransaction.getRequest().getNonce().setDescription (request.getPaymentNonce().getDescription());
                ltransaction.getRequest().getNonce().setNonce (request.getPaymentNonce().getNonce());
                ...
                */
                ...
                do
                {
                    ...
                    if (i < 1)
                    {
                        ltransaction.getRequest().setDescription (lineItem.getDescription());
                    }
                    // else do nothing
                ...
                ltransaction.getRequest().setAmount (Double.valueOf (request.getAccount().getAmount()));
                ltransaction.setRefId (request.getClientData().getGuid());
                ltransaction.getRequest().setAccountList (accounts);
                ltransaction.getRequest().setTransInvoiceNo (request.getAccount().getStatementNo());
//// IMPLEMENT FOR PHASE II
////                    ltransaction.getRequest().setDescription ("Payment for account " + account.getAccountNo()
////                                                             + ", Date: " + today.getTime()
////                                                             + ", Statement: " + request.getAccount().getStatementNo()
////                                                            );
//// PHASE II END
                ...
                ltransaction = methods.requestTransaction (ltransaction, params);
                lresponse.setGuid (ltransaction.getRefId());
                lresponse.setAuthCode (ltransaction.getResponse().getAuthCode());
                lresponse.setTransactionId (ltransaction.getResponse().getTransactionId());

                if (ltransaction.getRefId().equals (request.getClientData().getGuid())
                    && ltransaction.getResponse().getErrorCode() == TRANSACTION_SUCCESS)
                {
                    lresponse.setErrorMsg (ltransaction.getResponse().getErrorMessage());
                    lresponse.setErrorCode ("" + ltransaction.getResponse().getErrorCode());

                    if (ltransaction.getRequest().getCustomer() != null)
                    {
////                        lresponse.setCustAccount (ltransaction.getRequest().getUser().getAccountNum());
////                        lresponse.setProfileToken (ltransaction.getRequest().getCustomer().getProfileToken());
                    }
                }
                else
                {
